...
class PDFIntelligencePipeline:
    ...
    def __init__(self, output_root: Path | None = None) -> None:
        self.output_root = output_root or settings.pdf_intelligence_output_dir
        self.stage_dirs = {
        ...
        with fitz.open(str(pdf_path)) as doc:
            ...
            for index in range(len(doc)):
                ...
                filter_regions = [
                    ...
                    *self._detect_watermark_regions(text_dict, page),
    ...
    def run(
        ...
        *,
        ...
        logger.info("[pdf-intel] run start pdf=%s pages=%s", pdf_path.name, len(base_pages))

        stage_started = time.perf_counter()
        logger.info("[pdf-intel] stage0 preprocess start pdf=%s", pdf_path.name)
        stage0 = self.stage0_preprocess(pdf_path)
        logger.info(
            "[pdf-intel] stage0 preprocess done pdf=%s pages=%s elapsed_ms=%s",
            pdf_path.name,
            len(stage0),
        ...
        stage_started = time.perf_counter()
        logger.info("[pdf-intel] stage1 layout start pdf=%s", pdf_path.name)
        stage1 = self.stage1_layout(pdf_path, stage0, base_pages)
        logger.info(
            "[pdf-intel] stage1 layout done pdf=%s pages=%s elapsed_ms=%s",
            pdf_path.name,
            len(stage1),
            int((time.perf_counter() - stage_started) * 1000),
        )
        ...
        stage_started = time.perf_counter()
        logger.info("[pdf-intel] stage2 extract start pdf=%s", pdf_path.name)
        stage2 = self.stage2_extract(pdf_path, base_pages, stage1)
        logger.info(
            "[pdf-intel] stage2 extract done pdf=%s tables=%s figures=%s elapsed_ms=%s",
            pdf_path.name,
            len(stage2.get("tables", [])),
            len(stage2.get("figures", [])),
            int((time.perf_counter() - stage_started) * 1000),
        )

        stage_started = time.perf_counter()
        logger.info("[pdf-intel] stage3 context start pdf=%s", pdf_path.name)
        stage3 = self.stage3_context(base_pages, stage1, stage2)
        logger.info(
            "[pdf-intel] stage3 context done pdf=%s links=%s elapsed_ms=%s",
            pdf_path.name,
            len(stage3),
            int((time.perf_counter() - stage_started) * 1000),
        )
        ...
        stage_started = time.perf_counter()
        logger.info("[pdf-intel] stage4 multimodal start pdf=%s", pdf_path.name)
        stage4 = self.stage4_multimodal(base_pages, stage2, stage3)
        logger.info(
            "[pdf-intel] stage4 multimodal done pdf=%s facts=%s elapsed_ms=%s",
            pdf_path.name,
            len(stage4),
            int((time.perf_counter() - stage_started) * 1000),
        )
        logger.info(
            "[pdf-intel] run done pdf=%s elapsed_ms=%s",
            pdf_path.name,
