...
FIELD_EXPANSIONS: Dict[str, List[str]] = {
    ...
    "董事会秘书": ["董事会秘书", "公司董事会秘书", "董事会秘书姓名"],
    "证券事务代表": ["证券事务代表", "证券事务代表姓名", "董事会秘书及证券事务代表"],
    "流动覆盖率": ["流动覆盖率", "流动性覆盖率", "LCR", "流动性风险管理"],
...
def detect_preferred_sections(query: str, field_keys: List[str]) -> List[str]:
    ...
    section_map = {
        ...
        "董事会秘书": ["董事、监事、高级管理人员", "基本情况", "公司治理"],
        "证券事务代表": ["董事、监事、高级管理人员", "基本情况", "公司治理"],
        "流动覆盖率": ["财务会计信息", "监管指标", "流动性风险管理"],
...
def detect_query_tags(query: str, field_keys: List[str]) -> List[str]:
    ...
    if any(token in normalized for token in ["表格", "名单", "项目", "包括", "分别", "哪些", "列表"]):
        tags.append("list")
    if any(token in normalized for token in ["股数", "比例", "金额", "收入", "数量", "募集资金", "持股比例", "覆盖率", "LCR"]):
        tags.append("table")
...
def detect_question_type(query: str, field_keys: List[str], query_tags: List[str]) -> str:
    normalized = rewrite_query(query)
    if any(key in field_keys for key in ["法定代表人", "注册资本", "董事会秘书", "证券事务代表", "补充流动资金", "技术标准"]):
        return "field_lookup"
