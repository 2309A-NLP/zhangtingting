...
def upload_pdf(pdf_path: str | None) -> tuple[str, str, str, Any]:
    if not pdf_path:
        return "请先选择一个 PDF 文件。", "未上传", "", gr.update()
    with open(pdf_path, "rb") as handle:
        files = {"pdf": (os.path.basename(pdf_path), handle, "application/pdf")}
        response = requests.post(f"{API_BASE}/upload-pdf", files=files, timeout=None)
    response.raise_for_status()
    data = response.json()
    upload_id = str(data.get("upload_id") or "")
    filename = str(data.get("filename") or "")
    return (
        f"上传并入库完成：{filename}，共 {data['chunks']} 个分块，文本集合 {data['collection_name']}",
        filename,
        upload_id,
        _build_uploaded_dropdown_update(filename, upload_id, int(data.get("chunks") or 0)),
    )
...
def _build_answer_markdown(data: dict[str, Any], show_rewrite: bool) -> str:
    ...
    if not answer_text:
        ...
        if citations:
            ...
            page_label = f"第 {page_number} 页" if page_number not in (None, "") else "引用片段"
            answer_text = f"未生成最终答案，先返回最相关证据：{page_label}\n\n{preview[:300]}"
        else:
            answer_text = "本次请求已完成，但后端未返回可显示的答案。"
    lines = [answer_text]
    ...
    if show_rewrite:
        ...
        llm_status = str(data.get("llm_status") or "").strip()
        llm_error = str(data.get("llm_error") or "").strip()
        if answer_source or llm_status:
            lines.append(f"> 回答来源：{answer_source or '-'} | LLM状态：{llm_status or '-'}")
        if llm_error:
            lines.append(f"> LLM错误：{llm_error[:240]}")
...
def _format_uploaded_choices(health: dict[str, Any]) -> tuple[list[tuple[str, str]], str, str]:
    documents = list(health.get("uploaded_documents") or [])
    if not documents:
        return [], "", "未上传"
    choices = []
    active_id = ""
    ...
    for item in documents:
        ...
        time_label = datetime.fromtimestamp(uploaded_at).strftime("%Y-%m-%d %H:%M") if uploaded_at else "未知时间"
        filename = str(item.get("filename") or upload_id)
        chunks = int(item.get("chunks") or 0)
        status = str(item.get("status") or "ready")
        status_label = "处理中" if status == "processing" else "已完成"
        choices.append((f"{filename} | {time_label} | {chunks} chunks | {status_label}", upload_id))
        if not active_id:
            active_id = upload_id
            active_name = filename
    return choices, active_id, active_name


def _build_uploaded_dropdown_update(filename: str, upload_id: str, chunks: int) -> Any:
    if not upload_id:
        return gr.update(choices=[], value=None)
    return gr.update(
        choices=[(f"{filename} | 刚刚上传 | {chunks} chunks | 已完成", upload_id)],
        value=upload_id,
    )


def _select_uploaded_pdf(value: str | None, health: dict[str, Any] | None = None) -> tuple[str, str]:
    upload_id = str(value or "")
    if not upload_id:
        return "", "未上传"
    documents = list((health or {}).get("uploaded_documents") or [])
    for item in documents:
        if str(item.get("upload_id") or "") == upload_id:
            return upload_id, str(item.get("filename") or upload_id)
    return upload_id, upload_id


def _initial_health_state() -> tuple[Any, str, str]:
    try:
        health = _fetch_health()
    except Exception:
        return gr.update(choices=[], value=None), "", "未上传"
    choices, active_id, active_name = _format_uploaded_choices(health)
    return gr.update(choices=choices, value=active_id or None), active_id, active_name


def _query_backend(query: str, language: str, corpus: str, upload_id: str, session_id: str, enable_conversation: bool) -> dict[str, Any]:
    payload = {
        "query": query,
        "language": language,
        "use_llm": True,
...
def _render_history(state: dict[str, Any], show_rewrite: bool) -> ChatHistory:
    history: ChatHistory = []
...
def ask_question_stream(
...
) -> Generator[tuple[ChatHistory, str, str, str], None, None]:
    history: ChatHistory = list(chat_history or [])
    if not query.strip():
        yield history, "Please enter a question.", "", session_id
        return
    active_session_id = session_id or _new_session_id()
    pending_history = history + [{"role": "user", "content": query}, {"role": "assistant", "content": "Processing..."}]
    yield pending_history, "", "", active_session_id

    def task() -> tuple[ChatHistory, str, str, str]:
        data = _query_backend(query, language, corpus, upload_id, active_session_id, enable_conversation)
        answer_text = _build_answer_markdown(data, show_rewrite)
        if not answer_text.strip():
            answer_text = "The request completed, but no answer was returned."
    ...
    def placeholder() -> tuple[ChatHistory, str, str, str]:
...
def ask_audio_question_stream(
    ...
    show_rewrite: bool,
    chat_history: ChatHistory | None,
) -> Generator[tuple[ChatHistory, str, str, str, str], None, None]:
    history: ChatHistory = list(chat_history or [])
    active_session_id = session_id or _new_session_id()
    placeholder_history = history + [{"role": "assistant", "content": "Processing audio..."}]
    yield placeholder_history, "", "", "", active_session_id

    def task() -> tuple[ChatHistory, str, str, str, str]:
        transcript_data = _transcribe_audio(audio_path)
        transcript_text = str(transcript_data.get("text") or "").strip()
        if not transcript_text:
            return history, "", "", "No valid speech content recognized.", active_session_id
        query_data = _query_backend(transcript_text, language, corpus, upload_id, active_session_id, enable_conversation)
        answer_text = _build_answer_markdown(query_data, show_rewrite)
        if not answer_text.strip():
            answer_text = "The request completed, but no answer was returned."
        final_history = history + [{"role": "user", "content": f"[audio] {transcript_text}"}, {"role": "assistant", "content": answer_text}]
        return final_history, transcript_text, _format_citations(query_data.get("citations", [])), "", active_session_id

    def placeholder() -> tuple[ChatHistory, str, str, str, str]:
...
def start_new_conversation() -> tuple[ChatHistory, str, str, str, str]:
    return [], _new_session_id(), "", "", "已创建新会话。"


def clear_current_conversation(session_id: str) -> tuple[ChatHistory, str, str, str, str]:
    if session_id:
...
def reload_conversation(session_id: str, show_rewrite: bool, chat_history: ChatHistory | None = None) -> tuple[ChatHistory, str]:
    if not session_id:
        return list(chat_history or []), ""
    state = _fetch_conversation_state(session_id)
    rendered_history = _render_history(state, show_rewrite)
    if not rendered_history:
        return list(chat_history or []), state.get("current_company", "")
    return rendered_history, state.get("current_company", "")


def refresh_current_company(session_id: str) -> str:
    if not session_id:
        return ""
    try:
        state = _fetch_conversation_state(session_id)
        ...
        return ""
    return str(state.get("current_company", "") or "")


def submit_feedback(chat_history: ChatHistory | None, helpful: bool) -> str:
...
with gr.Blocks(title="PDF RAG QA") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            corpus_choice = gr.Radio(
                choices=[("默认招股书", "default"), ("已上传 PDF", "uploaded")],
                value="default",
                label="查询范围",
            )
            uploaded_pdf_select = gr.Dropdown(
                choices=[],
                value=None,
                label="已上传 PDF 选择",
                interactive=True,
    ...
    with gr.Accordion("PDF 上传", open=False):
        ...
        active_upload_id = gr.Textbox(label="当前上传 ID", value="", interactive=False)
    ...
    demo.load(_initial_health_state, outputs=[uploaded_pdf_select, active_upload_id, active_pdf])
    upload_button.click(upload_pdf, inputs=[upload_input], outputs=[upload_status, active_pdf, active_upload_id, uploaded_pdf_select])
    uploaded_pdf_select.change(
        lambda value: _select_uploaded_pdf(value, _fetch_health()),
        inputs=[uploaded_pdf_select],
        outputs=[active_upload_id, active_pdf],
    )
    ask_button.click(
        ask_question_stream,
        inputs=[query, language, corpus_choice, active_upload_id, session_state, enable_conversation, show_rewrite, chat_state],
        outputs=[chatbot, citations, status_box, session_state],
    ).then(lambda history: history, inputs=[chatbot], outputs=[chat_state]).then(
        refresh_current_company,
        inputs=[session_state],
        outputs=[current_company_box],
    ...
    audio_button.click(
        ...
        inputs=[audio_input, language, corpus_choice, active_upload_id, session_state, enable_conversation, show_rewrite, chat_state],
    ...
    refresh_button.click(
        reload_conversation,
        inputs=[session_state, show_rewrite, chat_state],
