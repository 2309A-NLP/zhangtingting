...
class RagasEvaluationRequest(BaseModel):
    input_csv: str = Field(..., min_length=1, description="CSV path that contains question and gold answer columns")
    output_csv: str = Field("", description="Optional CSV output path for ragas scores")
    dataset_jsonl: str = Field("", description="Optional JSONL output path for generated ragas dataset")
    summary_json: str = Field("", description="Optional JSON output path for evaluation summary")
    top_k: int = Field(5, ge=1, le=20)
    corpus: Literal["default", "uploaded"] = "default"
    upload_id: str = ""
    use_llm: bool = True
    metrics: List[str] = Field(
        default_factory=lambda: ["context_precision", "context_recall", "faithfulness"],
        description="Requested ragas metrics",
    )
...
class RagasEvaluationResponse(BaseModel):
    ...
    evaluated_rows: int
    skipped_rows: int
    corpus: Literal["default", "uploaded"]
    upload_id: str = ""
    use_llm: bool
