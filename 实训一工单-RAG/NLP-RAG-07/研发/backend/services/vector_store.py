...
class MilvusVectorStore:
    def __init__(self, uri: str, collection_name: str, dimension: int) -> None:
        ...
        self.collection_field_names: set[str] = set()
        try:
            self._connect()
            self.collection = self._ensure_collection()
            self.collection_field_names = {str(field.name) for field in getattr(self.collection.schema, "fields", [])}
        except Exception:
            self.connected = False
        ...
        if utility.has_collection(self.collection_name):
            ...
            try:
                for field in getattr(collection.schema, "fields", []):
                    if str(getattr(field, "name", "")) == "embedding":
                        dim_value = getattr(field, "params", {}).get("dim")
                        if dim_value:
    ...
    def _dense_search_milvus(self, query_embedding: List[float], candidate_limit: int) -> List[Dict[str, object]]:
        ...
        expected_dim = None
        try:
            for field in getattr(self.collection.schema, "fields", []):
                if str(getattr(field, "name", "")) == "embedding":
                    dim_value = getattr(field, "params", {}).get("dim")
                    if dim_value:
        ...
        if expected_dim and actual_dim != expected_dim:
            raise RuntimeError(
                "Milvus vector dimension mismatch before search: "
                f"collection={self.collection_name}, expected_dim={expected_dim}, actual_dim={actual_dim}. "
                "This usually means the current runtime loaded a different embedding model than the one used when the collection was built."
            )
        output_fields = [field for field in ["page_number", "logical_page", "text", "source_pdf", "page_start", "page_end", "doc_name", "metadata_json", "source_pages_json"] if field in self.collection_field_names]
        try:
            ...
            if "vector dimension mismatch" in message:
                raise RuntimeError(
                    "Milvus vector dimension mismatch during search: "
                    f"collection={self.collection_name}, expected_dim={expected_dim or self.dimension}, actual_dim={actual_dim}. "
                    "The collection was indexed with one embedding dimension, but the current process generated another. "
                    "Common causes: running evaluation in a different Python environment, sentence-transformers failed to load and the code fell back to hashing embeddings, or the embedding model path changed."
                ) from exc
            raise
        ...
        for hit in results[0]:
            ...
            metadata_json = hit.entity.get("metadata_json") if "metadata_json" in self.collection_field_names else ""
            if metadata_json:
                try:
                    ...
                    pass
            source_pdf = str(
                hit.entity.get("source_pdf")
                or hit.entity.get("doc_name")
                or metadata.get("source_pdf")
                or metadata.get("doc_name")
                or ""
            )
            page_number_value = hit.entity.get("page_number")
            if page_number_value is None:
                page_number_value = hit.entity.get("page_start")
            logical_page_value = hit.entity.get("logical_page") or metadata.get("logical_page")
            dense_candidates.append(
                {
                    "chunk_id": chunk_id,
                    "page_number": int(page_number_value or 0),
                    "logical_page": logical_page_value or None,
                    "text": local_record.get("text", hit.entity.get("text") or metadata.get("text") or ""),
                    "dense_score": float(hit.score),
                    "metadata": {**metadata, "source_pdf": source_pdf},
                }
            )
