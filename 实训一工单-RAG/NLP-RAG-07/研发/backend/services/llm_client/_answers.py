...
def _sanitize_context_text(text: str) -> str:
    ...
    if not cleaned:
        return ""
    cleaned = re.sub(r"\[(?:TABLE|IMAGE|VISUAL|FIGURE):[^\]]+\]", " ", cleaned, flags=re.IGNORECASE)
    cleaned = cleaned.replace("TABLE CONTENT:", "\n").replace("VISUAL CONTENT:", "\n")
    cleaned = cleaned.replace("猫隆篓忙聽录氓聠聟氓庐鹿茂录職", "\n")
    cleaned = cleaned.replace("氓聸戮猫隆篓氓聠聟氓庐鹿茂录職", "\n")
    cleaned = cleaned.replace("è¡¨æ ¼å å®¹ï¼", "\n").replace("å¾è¡¨å å®¹ï¼", "\n")
    cleaned = re.sub(r"\bVISUAL_TYPE:\s*[A-Za-z0-9_:-]+\b", " ", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\bPAGE:\s*\d+\b", " ", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\bSTRUCTURED_HEADER:\s*", "\n", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\bSTRUCTURED_ROW:\s*", "\n", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\bHEADER:\s*", "\n", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\bROW:\s*", "\n", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\b(?:VISUAL|TABLE|IMAGE|FIGURE)_TYPE:\s*[^\s]+\b", " ", cleaned, flags=re.IGNORECASE)
    return cleaned


def _clean_answer_text(answer: str) -> str:
    text = _sanitize_context_text(answer).strip()
    if not text:
        return text
    replacements = {
        "字段：": "",
    ...
    text = re.sub(r"\bVISUAL_TYPE:\s*[A-Za-z0-9_:-]+\b", "", text, flags=re.IGNORECASE)
    text = re.sub(r"\bPAGE:\s*\d+\b", "", text, flags=re.IGNORECASE)
    text = re.sub(r"\b(?:STRUCTURED_HEADER|STRUCTURED_ROW|HEADER|ROW):\s*", "", text, flags=re.IGNORECASE)
...
def _normalize_payload(text: str) -> str:
    return re.sub(r"\s+", " ", _sanitize_context_text(text or "")).strip()


def _context_lines(contexts: List[Dict[str, object]]) -> List[str]:
    lines: List[str] = []
    for item in contexts:
        cleaned = _sanitize_context_text(str(item.get("text") or ""))
...
def _extract_table_row(contexts: List[Dict[str, object]], labels: List[str]) -> List[str]:
    aliases = [label for label in labels if label]
    if not aliases:
        return []
    for line in _context_lines(contexts):
        if "|" not in line or not any(alias in line for alias in aliases):
            continue
        parts = [part.strip() for part in line.split("|") if part.strip()]
        if parts and any(alias in parts[0] or alias in line for alias in aliases):
            return parts
    return []


def _extract_contact_name(contexts: List[Dict[str, object]], role: str) -> str:
    payload = " ".join(_normalize_payload(str(item.get("text") or "")) for item in contexts)
    if role == "董事会秘书":
        patterns = [
            r"董事会秘书\s+证券事务代表\s+姓名\s+([\u4e00-\u9fff]{2,12})\s+[\u4e00-\u9fff]{2,12}",
            r"([\u4e00-\u9fff]{2,12})\s+董事会秘书\b",
        ]
    else:
        patterns = [
            r"董事会秘书\s+证券事务代表\s+姓名\s+[\u4e00-\u9fff]{2,12}\s+([\u4e00-\u9fff]{2,12})",
...
def _format_percent(value: str) -> str:
    ...
    if re.search(r"%$", normalized):
        return normalized
    if re.fullmatch(r"[<>≤≥]?\s*[0-9][0-9,\.]*", normalized):
        plain = normalized.replace("≥", "").replace("≤", "").strip()
        return f"{plain}%"
    return normalized


def _answer_common_table_metric(query: str, contexts: List[Dict[str, object]]) -> str:
    cost_income_row = _extract_table_row(contexts, ["成本收入比"])
    if "成本收入比" in query and cost_income_row:
        values = [part for part in cost_income_row[1:] if part]
        current = values[0] if values else ""
        yoy = values[-1] if len(values) >= 4 else ""
        current = _format_percent(current)
        if current and yoy:
            direction = "同比下降" if str(yoy).strip().startswith("-") else "同比上升"
            yoy_text = str(yoy).strip().lstrip("+-")
            return _clean_answer_text(f"2019年成本收入比为{current}，{direction}{yoy_text}。{_citation_text(contexts)}")
        if current:
            return _clean_answer_text(f"2019年成本收入比为{current}。{_citation_text(contexts)}")

    lcr_row = _extract_table_row(contexts, ["流动覆盖率", "流动性覆盖率"])
    if any(token in query for token in ["流动覆盖率", "流动性覆盖率", "LCR"]) and lcr_row:
        values = [part for part in lcr_row[1:] if part and "注" not in part and "适用" not in part]
        primary = ""
        for value in values:
            ...
            if re.search(r"\d", candidate):
                primary = candidate
                break
        primary = _format_percent(primary)
        if primary:
            prefix = "截至2019年12月31日，" if ("2019" in query and "12" in query) else ""
            return _clean_answer_text(f"{prefix}流动覆盖率为{primary}。{_citation_text(contexts)}")
    return ""
...
def _answer_field_lookup(query: str, contexts: List[Dict[str, object]], field_keys: List[str]) -> str:
    rules = {
        ...
        "董事会秘书": [r"董事会秘书[:：]?\s*([\u4e00-\u9fff]{2,12})"],
        "证券事务代表": [r"证券事务代表[:：]?\s*([\u4e00-\u9fff]{2,12})"],
        "流动覆盖率": [r"流动(?:性)?覆盖率[^0-9]{0,20}([0-9][0-9,\.]*%?)"],
    ...
    for field_key in field_keys:
        ...
        if field_key in {"董事会秘书", "证券事务代表"}:
            value = _extract_contact_name(contexts, field_key) or value
        if not value:
            continue
        if field_key == "法定代表人":
        ...
        if field_key == "董事会秘书":
            return _clean_answer_text(f"董事会秘书是{value}。{_citation_text(contexts)}")
        if field_key == "证券事务代表":
            return _clean_answer_text(f"证券事务代表是{value}。{_citation_text(contexts)}")
        if field_key == "流动覆盖率":
            value = _format_percent(value)
            return _clean_answer_text(f"流动覆盖率为{value}。{_citation_text(contexts)}")
        if field_key == "补充流动资金":
    ...
    metric_answer = _answer_common_table_metric(query, contexts)
    if metric_answer:
...
def _extractive_answer(self, query: str, contexts: List[Dict[str, object]], intent: object | None = None) -> str:
    ...
    metric_answer = _answer_common_table_metric(query, filtered_contexts)
    if metric_answer:
        return metric_answer
    ...
    for item in filtered_contexts[: settings.generation_top_n]:
        text = _sanitize_context_text(str(item["text"])).replace("|", "，").replace("\n", " ")
