...
def _subject_followup_question(company: str, subject: str) -> str:
    if not company or not subject:
        return ""
    suffix = "\u662f\u8c01" if subject in _PERSON_ROLE_SUBJECTS else "\u662f\u4ec0\u4e48"
    return f"{company}{subject}{suffix}"
...
def resolve_query(query: str, state: ConversationState | None) -> ResolvedQuery:
    ...
    if used_history and current_subject and current_subject not in rewritten_query:
        if re.search(r"(\u90a3\u4e2a|\u8fd9\u4e2a).*(\u5462|\u91cc)$", normalized) or normalized in {
            ...
            rewritten_query = _subject_followup_question(resolved_company, current_subject) or rewritten_query
    ...
    elif used_history and current_subject and normalized.endswith("\u5462"):
        followup_query = _subject_followup_question(resolved_company, current_subject)
        if followup_query:
