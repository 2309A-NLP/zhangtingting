...
class MongoTableWriter:
    def __init__(self, uri: str, db_name: str, collection_name: str) -> None:
        try:
            ...
            raise RuntimeError(
                "pymongo is required for table persistence. Install dependencies from requirements.txt or requirements_all.txt."
            ) from exc
...
def main() -> None:
    ...
    visual_upload_enabled = not args.skip_visual_upload
    artifact_upload_enabled = not args.skip_artifact_upload
    if visual_upload_enabled or artifact_upload_enabled:
        try:
            ...
            log(f"minio_upload_disabled={exc}")
            visual_upload_enabled = False
            artifact_upload_enabled = False
            summary["steps"]["minio_upload"] = {
                "enabled": False,
                "reason": str(exc),
            }

    visual_minio_map: dict[str, str] = {}
    if visual_upload_enabled:
    ...
    if uploader is not None and artifact_upload_enabled:
