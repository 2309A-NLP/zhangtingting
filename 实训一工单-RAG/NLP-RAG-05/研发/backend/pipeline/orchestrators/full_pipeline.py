...
from backend.pipeline.profiles import ProspectusProfile, get_prospectus_profile
...
def stage_script_path(project_root: Path, filename: str) -> Path:
    return ensure_exists(project_root / "backend" / "pipeline" / "stages" / filename, f"stage script {filename}")
