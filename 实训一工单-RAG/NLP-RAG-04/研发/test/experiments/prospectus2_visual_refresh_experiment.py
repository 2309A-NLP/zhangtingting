...
def log(message: str) -> None:
    ...
    print(f"[visual-refresh {timestamp}] {message}", flush=True)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rebuild prospectus2 visual tasks from existing stage1 layout, then rerun VLM/vectorize/persist."
    )
    ...
    parser.add_argument("--run-vlm", action="store_true", help="Run multimodal tasks after rebuilding visual tasks")
    parser.add_argument("--run-vectorize", action="store_true", help="Run visual vectorization after VLM")
    parser.add_argument("--run-persist", action="store_true", help="Persist visual vectors after vectorization")
    parser.add_argument("--overwrite-vlm", action="store_true", help="Overwrite existing visual VLM results")
...
def build_visual_tasks_from_stage1(
    ...
    crop_root = artifact_dir / "crops" / "visual_refresh"
    ...
    with fitz.open(str(pdf_path)) as doc:
        for page_info in stage1.get("page_details", []):
            ...
            for region in [*visual_regions, *short_text_regions]:
                region_id = str(region.get("region_id") or "")
            ...
            merged_region_id = f"p{page_index}_visual_refresh_merged"
            ...
            if not existing_visual_components:
            ...
            task = {
                "group_id": f"visual_refresh_page_{page_index:04d}",
                "page_index": page_index,
                "region_type": "visual_group",
                "object_type": "visual_group",
                "sub_type": "grouped_same_page_visuals_refresh",
                "route": "vlm_direct",
                ...
                "context_text": context_text,
                "refresh_method": "merged_visual_region_from_stage1",
...
def rewrite_artifact_files(artifact_dir: Path, visual_tasks: list[dict[str, Any]]) -> None:
    ...
    backup_root = artifact_dir / "visual_refresh_backup" / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    write_json(artifact_dir / "visual_refresh_manifest.json", refresh_manifest)
