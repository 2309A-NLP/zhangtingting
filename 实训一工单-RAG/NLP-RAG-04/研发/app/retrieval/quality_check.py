...
COMPANY_RE = re.compile(r"([\u4e00-\u9fffA-Za-z0-9()（）]{4,}?(?:股份有限公司|有限责任公司|集团股份有限公司|集团有限公司))")
...
MOJIBAKE_HINT_RE = re.compile(r"[锛銆鈥€鍥鏄鏈閿缁璇瀵鎺闆妗绫浠涓]")
...
def build_query_tokens(query: str) -> list[str]:
    ...
    tokens.extend(LATIN_TOKEN_RE.findall(normalized))
...
def build_test_llm_prompt(query: str, contexts: list[dict[str, Any]], per_context_limit: int = 1600) -> str:
    ...
    return (
        ...
        "不要输出内部标识、表格ID、chunk_id、marker、source_pdf、minio_path等内部字段。\n"
        "不要照抄大段原文，要整理成自然中文句子。\n"
...
def extract_company_aliases(query: str) -> list[str]:
    ...
    match = COMPANY_RE.search(sanitized_query)
    if not match:
        return []
    company = match.group(1).strip()
    aliases = [company]
    short_company = re.sub(r"(股份有限公司|有限责任公司|集团股份有限公司|集团有限公司)$", "", company).strip()
    if short_company and short_company not in aliases:
...
def compute_focus_signal(query: str, text: str) -> tuple[float, dict[str, Any]]:
    ...
    hits = [term for term in focus_terms if term in haystack]
    hit_ratio = len(hits) / max(1, len(focus_terms))

    score = 0.0
    if hits:
        score += min(0.16, 0.05 + 0.035 * len(hits))
    else:
        score -= 0.14

    if any(term in haystack for term in ["元", "万元", "亿元", "%"]) and any(
        token in normalize_text(query) for token in ["多少", "金额", "占比", "比例", "收入", "股数", "注册资本"]
    ):
        score += 0.03

    return score, {"focus_terms": focus_terms, "hits": hits, "hit_ratio": hit_ratio, "penalty": not bool(hits)}


def compute_company_signal(query: str, text: str, doc_name: str = "") -> tuple[float, dict[str, Any]]:
    aliases = extract_company_aliases(query)
    if not aliases:
        return 0.0, {"aliases": [], "matched": False}

    haystack = normalize_text(text)
    doc_haystack = normalize_text(doc_name)
    matched = any(alias and (alias in haystack or alias in doc_haystack) for alias in aliases)
    if matched:
        return 0.08, {"aliases": aliases, "matched": True}

    other_companies = COMPANY_RE.findall(haystack)
    if other_companies:
        return -0.08, {"aliases": aliases, "matched": False, "other_companies": other_companies[:3]}
...
def build_focus_snippet(query: str, text: str, max_sentences: int = 4) -> str:
    ...
    for index, sentence in enumerate(sentences):
        ...
        if any(token in query for token in ["多少", "金额", "收入", "比例", "占比", "股数"]) and re.search(
            r"\d[\d,]*(?:\.\d+)?(?:%|元|万元|亿元|万股|股)?",
            sentence,
        ):
            score += 2.0
...
def compute_page_position_penalty(
    ...
    *,
    ...
    pages = sorted({int(page) for page in source_pages if int(page) > 0})
...
def compute_answer_boost(query: str, text: str) -> float:
    ...
    if not normalized_text:
        return 0.0
    if any(token in normalized_query for token in ["多少", "几", "比例", "占比", "金额", "收入", "股数", "注册资本"]):
        if re.search(r"\d[\d,]*(?:\.\d+)?(?:%|万元|亿元|元|万股|股)?", normalized_text):
            bonus += 0.05
    if any(token in normalized_query for token in ["项目", "哪些", "包括", "分别", "关联方", "供应商"]):
        if any(token in normalized_text for token in ["包括", "如下", "分别", "项目", "合计", "指", "为"]):
            bonus += 0.05
    if any(token in normalized_query for token in ["图", "结构图", "流程图", "增长率", "负增长", "最快"]):
        if any(token in normalized_text for token in ["增长率", "最快", "负增长", "流程", "结构", "部门", "销售处"]):
            bonus += 0.04
...
def infer_question_type(query: str) -> str:
    normalized = normalize_text(query)
    if any(token in normalized for token in ["组织结构图", "流程图", "结构图", "销售部", "销售处"]):
        return "org_structure"
    if any(token in normalized for token in ["增长率", "负增长", "最快", "图中可以看出", "折线图", "柱形图"]):
        return "chart_trend"
    if any(token in normalized for token in ["占比", "比例", "分别是多少", "金额", "收入", "股数"]):
        return "table_numeric"
    if any(token in normalized for token in ["哪些", "包括", "分别", "项目", "关联方"]):
        return "table_list"
    if any(token in normalized for token in ["注册资本", "法定代表人", "技术标准", "一等奖", "供应商"]):
        return "field_lookup"
    return "fact_text"
...
def infer_query_tags(query: str) -> list[str]:
    ...
    pairs = [
        ...
        ("募投", "fundraising"),
        ("关联方", "related_party"),
        ("军用领域", "military_revenue"),
        ("技术标准", "technical_standard"),
        ("组织结构图", "org_chart"),
        ("增长率", "chart_trend"),
        ("上游", "industry_chain"),
        ("下游", "industry_chain"),
...
def generate_llm_answer_with_diagnostics(
    ...
    try:
        ...
        for attempt_index, plan in enumerate(retry_plan, start=1):
            ...
            cleaned = normalize_text(client._clean_answer_text(content or "")) if content is not None else ""
