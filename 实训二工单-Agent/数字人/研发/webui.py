...
if __name__ == "__main__":
    # 初始化LLM类
    # mode='offline'表示使用离线模式（不依赖API，使用本地模型或简单回复）。
    llm_class = LLM(mode='offline')
    # 调用init_model方法初始化一个具体的LLM模型实例。这里选择'直接回复 Direct Reply'模式，表示不使用真正的AI模型，只是简单回复或回显输入，这样可以节省显存。
    ...
    # max_size=4：队列最大长度为4
    # default_concurrency_limit=2：默认并发限制为2（同时最多处理2个请求）
